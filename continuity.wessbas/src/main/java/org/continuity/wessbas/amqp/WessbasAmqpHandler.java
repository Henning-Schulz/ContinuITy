...
public class WessbasAmqpHandler {
	...
	private void handleModelCreated(String storageId, WorkloadModel workloadModel, Date timestamp) {
		...
		String routingKey = "wessbas.wessbas" + RestApi.Wessbas.Model.OVERVIEW.path(storageId);
		amqpTemplate.convertAndSend(RabbitMqConfig.MODEL_CREATED_EXCHANGE_NAME, routingKey, responsePack);
	}
